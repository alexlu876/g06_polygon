...
void draw_polygons( struct matrix *polygons, screen s, color co ) {
  ...
  for(i = 0; i < polygons->lastcol; i+=3){
    ...
    double c;
    double d;
    a = polygons->m[0][i+1] - polygons->m[0][i];
    b = polygons->m[0][i+2] - polygons->m[0][i];
    c = polygons->m[1][i+1] - polygons->m[1][i];
    d = polygons->m[1][i+2] - polygons->m[1][i];
    double cross = (a * d) - (b * c);
    if(cross > 0){
      draw_line(polygons->m[0][i], polygons->m[1][i], polygons->m[0][i+1], polygons->m[1][i+1], s, co);
      draw_line(polygons->m[0][i+1], polygons->m[1][i+1], polygons->m[0][i+2], polygons->m[1][i+2], s, co);
      draw_line(polygons->m[0][i], polygons->m[1][i], polygons->m[0][i+2], polygons->m[1][i+2], s, co);
    }
  }
}
...
void add_sphere( struct matrix * edges, 
  ...
  for ( lat = latStart; lat < latStop; lat++ ) {
    for ( longt = longStart; longt < longStop; longt++ ) {
      ...
      a = (index + 1) % (points->lastcol);
      b = (index + step) % (points->lastcol);
      c = (index + step - 1) % (points->lastcol);
      
      add_polygon(edges, points->m[0][index], points->m[1][index], points->m[2][index], points->m[0][a], points->m[1][a], points->m[2][a], points->m[0][b], points->m[1][b], points->m[2][b]);
      add_polygon(edges, points->m[0][index], points->m[1][index], points->m[2][index], points->m[0][b], points->m[1][b], points->m[2][b], points->m[0][c], points->m[1][c], points->m[2][c]);
    }
  }
